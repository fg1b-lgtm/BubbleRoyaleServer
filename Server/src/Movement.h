                      ...
                      int step, bool moving_is_x, int speed)
{
    ...
    // 줄에 맞추는 일을 도중에 끊지 않는다.
    //
    // 9/2 에는 좁은 데로 들어갈 때, 그중에서도 몸이 실제로 안 들어갈 때만 당겼다.
    // 매 틱 가운데로 끌던 것보다는 나았는데도 밀린다는 신고가 안 없어졌다.
    // 재보니 이유가 나왔다. 판 20개를 326,144번 걸었더니 당기다 끊기고 다시
    // 당기기를 40,534번 했다. 방향이 뒤집힌 적은 한 번도 없었으니, 잘못된 쪽으로
    // 민 게 아니라 그냥 켜졌다 꺼졌다 한 것이다.
    //
    // 기둥을 지날 때마다 조건이 바뀌니 그럴 수밖에 없었다. 몸이 두 줄에 걸치면
    // 옆줄 기둥이 걸려서 당기고, 한 줄 안에 들어오면 안 당긴다. 그게 되풀이된다.
    // 사람 손에는 그 껐다 켜짐이 밀린다고 느껴진다.
    //
    // 그래서 조건을 없앴다. 한 축으로 걷는 동안에는 늘 줄 가운데로 당긴다.
    // 그러면 걷기 시작하고 몇 틱이면 줄에 붙고, 그 뒤로는 당길 일이 없으니
    // 아무 일도 안 일어난다. 한 번 쭉 맞춰주는 것과 계속 들썩이는 것은 다르다.
    //
    // 당기는 속도도 올렸다. 예전에는 걷는 속도의 절반이라 줄에 붙는 데 여덟 틱이
    // 걸렸고 그 여덟 틱이 눈에 보였다. 걷는 속도와 같게 하면 서너 틱이면 끝난다.
    //
    // 이 게임은 어느 칸에 물풍선을 놓느냐가 판단의 전부다. 줄에 붙어 있으면
    // 조준하기도 쉬워진다. 크아가 같은 이유로 줄을 맞춰준다

...
//
// 조각은 손으로 그린 마을·사막·얼음골 3종에서 온다 (SectorTemplates.h).
// 확률로만 만들던 때는 규칙은 맞는데 어디를 가도 같은 데 같았다.
// 손으로 그린 조각에는 광장과 골목과 문턱이 있어서 "거기" 라고 부를 수 있다.
//
// 이 파일이 하는 일은 셋이다.
//   ① 3종을 아홉 자리에 반복해 뽑고, 뒤집어 가며 3x3 으로 붙인다
...
struct GameMap
{
    ...
    void Generate(unsigned int s, int block_percent = BLOCK_FILL_PERCENT)
    {
        ...
        //    조각이 있는 테마 수와 다를 수 있다 — 지금 마을·사막·얼음골 세 개만
        ...
        //
        // 조각이 많아져도 새로 그린 하나가 가운데에 올 때까지 판을 반복해서
        // 까는 것은 확인이 아니라 도박이다.
                     ...
                     MapRandom& rnd, int block_percent)
    {
        ...
        for (int ly = 0; ly < SECTOR_H; ++ly) {
            ...
            for (int lx = 0; lx < SECTOR_W; ++lx) {
                ...
                //
                // 사람이 그려온 판(마을·사막·얼음골)은 상자 하나하나가 그 자리에 있으라고

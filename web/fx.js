...
const FX = (() => {
  ...
  return {
    ...
    // 아이템이 물줄기에 쓸려갔다.
    //
    // 먹은 것과 다르게 보여야 한다. 먹은 건 위로 튀어올라 사라지고(가져갔다),
    // 이건 **그 자리에서 옆으로 흩어진다**(아무도 못 가졌다).
    // 물에 씻겨 내려간 것이라 색도 물색으로 쓴다
    gone(x, y, T, now) {
      for (let i = 0; i < 7; ++i) {
        const a = Math.random() * 6.28;
        emit({ x, y, vx: Math.cos(a) * T * 0.05, vy: Math.sin(a) * T * 0.05 - 0.02,
               r: T * rnd(0.04, 0.09), r1: 0,
               color: 'rgba(150,215,255,0.65)',
               drag: 0.90, life: rnd(220, 360), now, shape: 'soft' });
      }
    },

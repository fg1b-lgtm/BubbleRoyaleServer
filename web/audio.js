...
const Sound = (() => {
  ...
  return {
    ...
    // 대쉬. **바람 빠지는 소리**다.
    //
    // 새 녹음을 안 받는다. 있는 것으로 만든다 — 발소리를 아주 낮게 내리고
    // 높은 대역만 남긴 것을 겹치면 '슉' 이 된다. 짧게 끊어야 빠른 것으로 들린다.
    // 길게 끌면 미끄러지는 소리가 되고, 그러면 대쉬가 둔해 보인다
    dash(pan, far) {
      play('step',  { pan, far, gain: 0.34, rate: rnd(2.1, 2.4),
                      cut: 1100, filter: 'highpass' });
      play('crate', { pan, far, gain: 0.14, rate: rnd(1.7, 2.0),
                      cut: 900,  filter: 'highpass' });
    },

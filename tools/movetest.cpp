...
static void Test4_Slide()
{
    ...
    // 9/4: 대각선 두 축을 각자 speed 만큼 다 보내던 걸 고쳤다(Game.h).
    // 대각선이 직선보다 약 1.41배 빠르게 가던 게 그 증거였는데,
    // 거꾸로 말하면 이 시험도 그 빠른 속도에 맞춰 60틱으로 잰 것이었다.
    // 규칙(1/√2)을 바로잡았으니 벽까지 닿는 데 걸리는 틱도 늘어난다 -
    // 90틱으로 늘려서 같은 자리(가로 타일 7)에 닿는지를 다시 잰다
    int start_y = p.py;
    for (int t = 0; t < 90; ++t) {
        MovePlayer(m, p);
    }

...
for (let y = 0; y < MAP_H; ++y) {
    // 맵 줄은 타일 · 아이템 · 길 · 겉모습 네 줄이다.
    //   길     조각을 그릴 때 '반드시 통로' 로 그은 자리. 흙길을 그리는 데 쓴다
    //   겉모습 같은 벽인데 다르게 보여야 하는 것. 강(1)과 다리(2)
    feed(pkt(6, 1 + MAP_W * 4, (v, o) => {
        v.setUint8(o, y);
        ...
        for (let x = 0; x < MAP_W; ++x) {
            // 겉모습. 세로로 강 한 줄을 내고 그 위에 다리를 하나 놓는다.
            // 강은 규칙으로는 벽이라 타일도 벽이어야 앞뒤가 맞는다
            const river = (x === 20 && !(x % 2 === 0 && y % 2 === 0));
            v.setUint8(o + 1 + MAP_W * 3 + x, river ? (y === 9 ? 2 : 1) : 0);
        }
    }));
}

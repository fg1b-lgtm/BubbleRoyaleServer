...
//   15x13 -> 13x11 -> 11x9 -> 9x7 -> 7x5 -> 5x3 -> 3x1 -> 1x0 -> 안전한 데가 없다
//
// 7x5 에서 멈추게 해뒀다. 한 명당 12칸이면 반드시 만난다고 봤는데 아니었다.
// 봇 60판에서 9판이 10분을 넘겼다. 35칸 안에도 부술 수 있는 블록이 남아 있고,
// 서로 반대편 구석에서 각자 블록을 부수고 있으면 영영 안 만난다.
//
// **줄어들다 멈추는 것 자체가 문제**다. 멈추는 순간 그 판은 끝날 이유가 없어진다.
// 그래서 끝까지 줄인다. 마지막에는 안전한 칸이 하나도 안 남고 전원이 잠긴다.
//
// 잔인해 보이지만 이게 맞다. 마지막까지 안 만나기로 한 둘에게 무승부를 주면
// 다음 판에도 안 만난다. 물이 다 차기 전에 만나는 것이 유일한 길이어야 한다.
// 실제로는 여기까지 오는 판이 드물다 - 평균 3분 45초에 끝난다.
//
// 20초에서 15초로 줄였다. 끝까지 가는 데 20초면 2분이 더 걸린다
constexpr int RING_STEP_TICKS = TICK_RATE * 15;

// 칸의 겉모습.
//
// 규칙은 tile 하나로 돌아간다 (빈칸 · 벽 · 블록 · 상자). 그런데 사람이 그려온
// 판에는 **같은 벽인데 다르게 보여야 하는 것**이 있다. 강은 벽이지만 돌이
// 아니고, 다리는 빈칸이지만 잔디가 아니다.
//
// 규칙에 강을 넣지 않는다. 그러면 지나갈 수 있나 없나를 두 군데서 보게 된다.
// 강은 규칙에서 그냥 벽이고, 여기 붙는 번호가 "그 벽은 물로 그려라" 만 말한다.
//
// 조각 글자로는 ~ 가 물, = 이 다리다
enum TileLook : uint8_t
{
    LOOK_PLAIN  = 0,   // 그 장소의 기본 그림
    LOOK_WATER  = 1,   // 벽인데 물이다. 강 · 연못
    LOOK_BRIDGE = 2,   // 빈칸인데 다리다. 강을 건너는 자리
};
